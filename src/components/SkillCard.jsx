...
const SkillCard = ({skill, percentaje, color, move}) => {
  return (
    <div 
      className={"border border-[rgba(255,255,255,0.3)] rounded-md px-8 py-8 cursor-pointer group w-full md:w-[90%] flex items-center justify-between relative gap-2"}
      style={{
      ...
      <div 
        ...
        <motion.div
          initial={{ height: 0}}
          animate={{ height: percentaje}}
          transition={{ duration: 2.5}}
          style={{background: color}}
          className="w-full absolute bottom-0 left-0 flex items-center justify-center">
          <p className="text-bgPrimary z-10 font-sans font-bold tracking-wider">{percentaje}</p>
        </motion.div>
      </div>
    </div>
  );
